...
- [ ] **Phase 3 — Marketing, one platform end to end.** Instagram/Meta first. Fetch, store into `posts` and `post_metrics`, and surface reach, engagement rate, top posts, and follower trend. Fix the `instagram.js` issues here. Prove the pattern once. Correction from Phase 1: `posts`, `post_metrics`, and `platform_accounts` do not exist in the live DB and must be created here, not ported. From Phase 2: `platform_links` is now user-editable in the app and is the source of truth for which posts belong to which property; its `post_id` values are stored as `''` and must be resolved from the URLs here. Also owed from Phase 1: a platform trend line chart, once this phase starts accumulating `analytics_history` snapshots. Consider a small lead-capture form here too; every sales widget is waiting on lead rows.
...
- [ ] **Phase 5 — Polish.** Global date-range filter, per-widget drill-down, PDF export, loading and empty states, and auth if the app goes beyond single-user. (Listings editing, originally slated here, shipped in Phase 2.)
